...
import { tablePages } from "@@/src/constant/table";
...
import { type PagesType, PagesModel} from "../data/PagesModel";
...
export default function Pages() {
  ...
  const statename: string = 'pages'
  const windowWidth = useWindowSize();
  const router = useRouter()

  const initialMount = useCallback(async () => {
    let defaultValue: StateType<PagesType> = {
      isLoading: false,
      headers: tablePages,
      filter: [],
      filterKey: [
      ...
      bulk: [
        {
          action: (id, index) => {
            router.push(`/admin/pages/update/${id}`)
          },
          name: 'Edit',
        ...
        {
          action: (id, index) => {
            router.push(`/admin/pages/view/${id}`)
          },
          name: 'View',
        ...
        },
        {
          action: async (id, index) => {
            const result: ApiResponse<{ id: string }> = await fetchClient('DELETE', '/data/pages/' + id)
            if(result.success){
      ...
      groupBy: "created_at",
      onGet: async (newObj: StateType<PagesType>) => {
        setState((prev) => {
          return {
            ...prev,
            [statename]: {
        ...
        const result: ApiResponse<TableResponse<PagesType[]>> = await fetchClient('GET', route + parameter)
        const responseData: TableResponse<PagesType[]> = result.data
        const value: PagesType[] = PagesModel.toDatatableResponse(responseData.data)
        setState((prev: any) => {
          return {
            ...prev,
            [statename]: {
  ...
  return (
    <div className="w-full h-full overflow-hidden flex flex-col">
      <div className="pb-5 px-5">
        <h1 className="font-bold text-xl border-l-4 border-primary-500 px-2">Pages Items</h1>
        ...
        <div className="flex md:items-center md:justify-between mt-5">
          ...
          <div className="w-full md:w-1/2 flex items-center justify-end gap-2">
            ...
            <Link href={`/admin/pages/create`} className="inline-block">
