...
import { tableClient, tableMenus } from "@@/src/constant/table";
...
import { ClientModel, ClientType } from "../data/ClientModel";

export default function ClientPages() {
  const { state, setState } = useGlobalContext();
  const statename: string = 'client'
  const windowWidth = useWindowSize();
  const router = useRouter()

  const initialMount = useCallback(async () => {
    let defaultValue: StateType<ClientType> = {
      isLoading: false,
      headers: tableClient,
      filter: [],
      filterKey: [
      ...
      bulk: [
        ...
        },
        {
          action: async (id, index) => {
            const result: ApiResponse<{ id: string }> = await fetchClient('DELETE', '/data/client/' + id)
            if(result.success){
      ...
      groupBy: "created_at",
      onGet: async (newObj: StateType<ClientType>) => {
        setState((prev) => {
          return {
            ...prev,
            [statename]: {
        ...
        const route: string = '/data/client'
        ...
        const result: ApiResponse<TableResponse<ClientType[]>> = await fetchClient('GET', route + parameter)
        const responseData: TableResponse<ClientType[]> = result.data
        const value: ClientModel[] = ClientModel.toDatatableResponse(responseData.data)
        setState((prev: any) => {
          return {
            ...prev,
            [statename]: {
  ...
  return (
    <div className="w-full h-full overflow-hidden flex flex-col">
      <div className="pb-5 px-5">
        <h1 className="font-bold text-xl border-l-4 border-primary-500 px-2">Client</h1>
        ...
        <div className="flex md:items-center md:justify-between mt-5">
          ...
          <div className="w-full md:w-1/2 flex items-center justify-end gap-2">
            ...
            <Link href={`/admin/client/create`} className="inline-block">
